...
void test_myIP_buff_unroll(int blockNum)
{
	...
	memset(dst64, 0, BLOCK_SIZE * blockNum);
		...
		for (int i = 0; i < TEST_NUM; i++) {
			counter.start();
			myIP_buff2_unroll16_cast_64(dst64, src64, blockNum);
			counter.stop();
		}
		std::cout << "myIP_buff2_unroll16_cast_64" << ": " << counter.avg_time() * 1000  << " msec" << std::endl;
		verifyResult((uint8_t*)dst64, (uint8_t*)src64, blockNum);
	...
	for (int i = 0; i < TEST_NUM; i++) {
		counter.start();
		myIP_buff2_unroll16_unroll16_cast_64(dst64, src64, blockNum);
		counter.stop();
	}
	std::cout << "myIP_buff2_unroll16_unroll16_cast_64" << ": " << counter.avg_time() * 1000  << " msec" << std::endl;
	verifyResult((uint8_t*)dst64, (uint8_t*)src64, blockNum);
	...
	for (int i = 0; i < TEST_NUM; i++) {
		counter.start();
		myIP_buff2_inner_unroll16_cast_64(dst64, src64, blockNum);
		counter.stop();
	}
	std::cout << "myIP_buff2_inner_unroll16_cast_64" << ": " << counter.avg_time() * 1000  << " msec" << std::endl;
	verifyResult((uint8_t*)dst64, (uint8_t*)src64, blockNum);
	...
	for (int i = 0; i < TEST_NUM; i++) {
		counter.start();
		myIP_partition_0_unroll16_mask_64(dst64, src64, blockNum);
		counter.stop();
	}
	std::cout << "myIP_partition_0_unroll16_mask_64" << ": " << counter.avg_time() * 1000  << " msec" << std::endl;
	verifyResult((uint8_t*)dst64, (uint8_t*)src64, blockNum);
	...
	for (int i = 0; i < TEST_NUM; i++) {
		counter.start();
		myIP_partition_4_unroll16_mask_64(dst64, src64, blockNum);
		counter.stop();
	}
	std::cout << "myIP_partition_4_unroll16_mask_64" << ": " << counter.avg_time() * 1000  << " msec" << std::endl;
	verifyResult((uint8_t*)dst64, (uint8_t*)src64, blockNum);
