...
void test_myIP_stream(int blockNum)
{
	...
	for (int i = 0; i < TEST_NUM; i++) {
		counter.start();
		myIP_stream_df_64(dst64, src64, blockNum);
		counter.stop();
	}
	std::cout << "myIP_stream_df_64" << ": " << counter.avg_time() * 1000  << " msec" << std::endl;
	...
	for (int i = 0; i < TEST_NUM; i++) {
		counter.start();
		myIP_stream_df_all_64(dst64, src64, blockNum);
		counter.stop();
	}
	std::cout << "myIP_stream_df_all_64" << ": " << counter.avg_time() * 1000  << " msec" << std::endl;
	verifyResult((uint8_t*)dst64, (uint8_t*)src64, blockNum);
